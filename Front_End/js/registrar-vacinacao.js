(function(){
  ...
  vacinaNomeInput.addEventListener("input", () => {
    ...
    debounceId = setTimeout(() => {
      ...
      // limpa UUID por enquanto (vai ser resolvido no change/selection)
      vacinaUuidInput.value = "";
      // se houver apenas 1 opção filtrada, pré-seleciona o uuid
  ...
  vacinaNomeInput.addEventListener("change", () => {
    ...
    const opts = Array.from(listaVacinas.options);
    let found = opts.find((o) => o.value.toLowerCase() === val); // exata
    if (!found) found = opts.find((o) => o.value.toLowerCase().includes(val)); // parcial
    if (!found && opts.length === 1) found = opts[0]; // se só tiver 1, usa ela
    if (found) {
      ...
      vacinaUuidInput.value = "";
      console.warn("Vacina nao encontrada na lista:", vacinaNomeInput.value);
    }
  });

  // também marca uuid se o usuário de fato escolheu uma option exata (alguns navegadores)
  vacinaNomeInput.addEventListener("input", () => {
    const opts = Array.from(listaVacinas.options);
  ...
  document
    ...
    .addEventListener("submit", async function (e) {
      ...
      const dataProximaDose = proximaRaw
        ? formatDateToDDMMYYYY(proximaRaw)
        : null;

      // Resolver pessoa por CPF
      const token = localStorage.getItem("token");
      if (!token) {
      ...
      try {
        ...
        if (!resp.ok) {
          alert(
            `Erro ao registrar vacinação: ${data?.mensagem || resp.status}`
          );
          return;
        }
